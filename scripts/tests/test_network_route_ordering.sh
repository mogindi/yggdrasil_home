...
grep -Fqx 'Restart=on-failure' "$veth_script"
grep -Fqx 'RestartSec=5s' "$veth_script"
grep -Fq 'veth1_master=' "$veth_script"
grep -Fq 'ip link set dev veth1 master br0' "$veth_script"
...
grep -Fq 'Requires=sys-subsystem-net-devices-openstack_mgmt.device' "$route_table_script"
