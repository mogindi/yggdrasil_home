...
for prov_net in $(echo ${NETWORK_PROVIDER_NETS} | xargs); do
rules=(
  "${rules[@]}"
  "\-A INPUT -s $prov_net -p tcp -m tcp --dport 22 -j DROP -m comment --comment 'block ssh from $prov_net'"
  "\-A INPUT -s $prov_net -j ACCEPT -m comment --comment 'allow traffic from deployment node $prov_net'"
)
done
