...
grep -Fq -- '--device "eth0,network=$LXC_NETWORK"' "$HOST_SCRIPT" || fail "LXD NIC properties are not passed separately"
grep -Fq -- 'wait_for_lxc_cloud_init' "$HOST_SCRIPT" || fail "LXD cloud-init wait is not restart-safe"
